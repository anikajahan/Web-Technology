...
if(isset($_POST["query"]))
{
	...
	if($_POST["query"] != '')
	{
		...
		$query = "
		SELECT id, fname, email, nid, gender FROM t_registration WHERE fname LIKE :fname OR email LIKE :email OR nid LIKE :nid OR gender LIKE :gender ORDER BY id DESC ";
		...
		foreach($result as $row)
		{
			$data[] = array(
				'id'			=>	$row["id"],
				'fname'		=>	str_ireplace($replace_array_1, $replace_array_2, $row["fname"]),
	...
	else
	{
		...
		$query = "
		SELECT id, fname, email, nid, gender FROM t_registration ORDER BY id DESC ";
